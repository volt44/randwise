...
function LoginScreen() {
  ...
  return (
    <div style={{
      fontFamily: FONT, background: C.bg, minHeight: "100dvh", color: C.text,
        ...
        <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 16, padding: 32 }}>
          ...
          {step === "sent" && (
            <div style={{ textAlign: "center" }}>
              <div style={{
                width: 48, height: 48, borderRadius: "50%", background: C.accentDim,
              ...
              <button onClick={() => { setStep("enter"); setEmail(""); }}
                style={{
                  marginTop: 24, background: "none", border: `1px solid ${C.border}`,
          ...
          {step === "error" && (
            <div style={{ textAlign: "center" }}>
              ...
              <button onClick={() => { setStep("enter"); setErrMsg(""); }}
                style={{
                  background: C.accentDim, border: `1px solid ${C.accentMid}`, color: C.accent,
...
function Dashboard({ user, onSignOut }) {
  ...
  const saveIncome = useRef(debounce(async (id, amount) => {
    setSyncStatus("saving");
    const { error } = await supabase.from("income").update({ amount }).eq("id", id);
    error ? setSyncStatus("error") : showSaved();
  }, 600)).current;

  const saveCategory = useRef(debounce(async (cat) => {
    setSyncStatus("saving");
    const { error } = await supabase.from("categories").upsert({ ...cat, user_id: user.id });
    error ? setSyncStatus("error") : showSaved();
  }, 600)).current;

  const saveDebt = useRef(debounce(async (debt) => {
    setSyncStatus("saving");
...
Total debt:${fmt(totalDebt)} | ${debts.map(d => `${d.name} ${fmt(d.balance)} @${d.rate}%`).join("; ")}`;
  ...
  if (dbStatus === "loading") return (
    <div style={{
      fontFamily: FONT, background: C.bg, minHeight: "100dvh", display: "flex",
        ...
        <div style={{ flex: 1, overflowY: tab === "advisor" ? "hidden" : "auto", display: "flex", flexDirection: "column" }}>
                ...
                <div className="card" style={{ borderColor: C.warn + "50" }}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(220px,1fr))", gap: 8 }}>
                    {categories.filter(c => c.spent > c.budget).map(c => (
                      <div key={c.id} style={{
                        background: C.warnDim, border: `1px solid ${C.warn}40`, borderRadius: 8,
          ...
          {tab === "budget" && (
            <Page>
              ...
              {["needs", "wants", "savings"].map(group => (
                <div key={group}>
                  <div style={{
                    fontFamily: "'Syne',sans-serif", fontWeight: 700, fontSize: 10, color: C.muted,
                            ...
                            {[{ l: "Budget", k: "budget", v: cat.budget, w: false }, { l: "Spent", k: "spent", v: cat.spent, w: over }].map(({ l, k, v, w }) => (
                              <div key={k}>
                                ...
                                <input type="number" value={v}
                                  ...
                                  style={{
                                    background: C.surface, border: `1px solid ${w ? C.warn : C.border}`,
          ...
          {tab === "debts" && (
            <Page>
                    ...
                    <div key={debt.id} className="card" style={{ borderColor: isFirst ? C.accentMid : C.border }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10, flexWrap: "wrap", gap: 8 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                          <div style={{
                            width: 28, height: 28, borderRadius: "50%", background: isFirst ? C.accentDim : C.surface,
                            ...
                            fontSize: 12, color: isFirst ? C.accent : C.muted, flexShrink: 0
                          }}>{i + 1}</div>
                          <div>
              ...
              <div style={{
                padding: "12px 16px", background: C.accentDim, border: `1px solid ${C.accentMid}`,
          ...
          {tab === "advisor" && (
            <div style={{ display: "flex", flexDirection: "column", flex: 1, overflow: "hidden", height: "calc(100dvh - 54px)" }}>
              <div style={{
                padding: "11px 20px", borderBottom: `1px solid ${C.border}`, background: C.surface,
              ...
              <div className="advisor-layout" style={{ flex: 1, overflow: "hidden", padding: "0 20px", marginTop: 16 }}>
                  ...
                  <div style={{ flex: 1, overflowY: "auto", padding: "18px" }}>
                    {chat.map((m, i) => (
                      <div key={i} className="up" style={{
                        ...
                        <div style={{
                          maxWidth: "80%", background: m.role === "user" ? C.accentDim : C.surface,
                ...
                <div className="advisor-sidebar" style={{ display: "flex", flexDirection: "column", gap: 12, overflowY: "auto" }}>
                  <div className="card">
                    <div style={{
                      fontFamily: "'Syne',sans-serif", fontWeight: 700, fontSize: 11, color: C.muted,
